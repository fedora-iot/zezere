...
class PortalSSHKeyTest(TestCase):
    def test_key_list(self):
        with self.loggedin_as():
            resp = self.client.get("/portal/sshkeys/", follow=True)
            self.assertEqual(len(resp.context["sshkeys"]), 0)

    def test_add_empty_key(self):
        with self.loggedin_as():
            resp = self.client.post("/portal/sshkeys/add/", {"sshkey": ""}, follow=True)
            self.assertEqual(len(resp.context["sshkeys"]), 0)

    def test_add_key(self):
        with self.loggedin_as():
            resp = self.client.post(
                "/portal/sshkeys/add/", {"sshkey": "somekeyvalue<script>"}, follow=True
            )
            self.assertEqual(len(resp.context["sshkeys"]), 1)
            # Check that we don't print the key back
            self.assertNotContains(resp, "<script>")

            self.assertEqual(resp.context["sshkeys"][0].key, "somekeyvalue<script>")
    ...
    def test_remove_key(self):
        with self.loggedin_as():
            ...
            resp = self.client.post(
                "/portal/sshkeys/delete/", {"sshkey_id": key.id + 5}, follow=True
            )
            self.assertIsInstance(resp, HttpResponseNotFound)

            resp = self.client.post(
                "/portal/sshkeys/delete/", {"sshkey_id": key.id}, follow=True
            )
    ...
    def test_other_person_key(self):
        ...
        with self.loggedin_as(self.USER_2):
            resp = self.client.post(
                "/portal/sshkeys/add/", {"sshkey": "someotherkey"}, follow=True
            )
            self.assertEqual(len(resp.context["sshkeys"]), 1)
            ...
            resp = self.client.post(
                "/portal/sshkeys/delete/", {"sshkey_id": user1key.id}, follow=True
            )
            self.assertIsInstance(resp, HttpResponseNotFound)
